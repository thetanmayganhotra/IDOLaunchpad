...
contract Pool is IPool, Ownable {
  ...
  constructor(PoolModel memory _pool, address _sToken) {
    ...
    returns (uint256 _tokensAmount)
  {
    ...
    uint256 _tokensAmount = totalRaised - amountParticipated;
  }

  function withdraw(address _participant)
    external
    override
    onlyOwner
    _isPoolFinished(poolInformation)
  ...
  {
    uint256 amountParticipated = collaborations[_participant];
    sToken.transfer(_participant,amountParticipated);
  }
